...
namespace ComputerServiceOnlineShop.Services
{
    public class AddressService : IAddressService
    {
        ...
        public async Task<EditAddressViewModel> GetAddressForEdit()
        {
            ...
            return await _databaseContext.Users.Where(item => item.IsActive && item.Id == userId)
                ...
                .Select(item => new EditAddressViewModel()
                {
                    Id = item.AdressId,
        ...
        public async Task<UserAddressDetailsViewModel> GetUserAddresInfo()
        {
            var userId = _accountService.GetLoggedUserId();
            return await _databaseContext.Users.Where(item => item.IsActive && item.Id == userId)
                .Include(item => item.Address)
                .Select(item => new UserAddressDetailsViewModel()
                {
